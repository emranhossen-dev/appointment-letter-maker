...
export const TechEnterpriseNoSigTemplate: React.FC<TemplateProps> = ({ data, onUpdate }) => {
  const { company, employee, compensation, clauses, style, customSubject, customGreeting, customOpeningParagraph, customClosing } = data;
  const primaryColor = style.primaryColor || '#0369a1'; // Deep Cyan / Tech Blue
  ...
  return (
    <div className="a4-sheet full-bleed text-slate-800 relative bg-white flex flex-col justify-between p-0 shadow-2xl font-sans overflow-hidden h-[297mm] min-h-[297mm] max-h-[297mm] box-border">
      ...
      <div className="p-6 sm:p-8 flex-1 flex flex-col justify-between z-10 relative">
        <div>
          {/* Dual Color Tech Top Banner Line */}
          <div className="flex -mx-6 -mt-6 sm:-mx-8 sm:-mt-8 mb-4">
            <div className="h-2.5 w-4/5" style={{ backgroundColor: primaryColor }} />
            <div className="h-2.5 w-1/5 bg-cyan-400" />
          </div>
          ...
          {/* Dark Pill Metadata Banner (Signature-Free Layout) */}
          <div className="bg-slate-900 text-white px-4 py-2.5 rounded-lg text-xs font-medium mb-4 shadow-sm flex items-center justify-between">
            <div>
              <span className="text-slate-400 uppercase tracking-wider text-[10px]">{isBn ? 'প্রাপক:' : 'RECIPIENT:'} </span>
              <span className="font-bold text-cyan-300 text-xs"><EditableText value={employee.name} onChange={(v) => updateEmployee('name', v)} /></span>
            </div>
            <div>
              <span className="text-slate-400 uppercase tracking-wider text-[10px]">{isBn ? 'পদবী:' : 'DESIGNATION:'} </span>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-800 leading-relaxed">
            ...
            {/* Closing Note (Signature-Free) */}
            <div className="space-y-2 text-xs sm:text-sm text-slate-800 leading-relaxed mt-3">
              <p>
                <EditableText 
                  multiline 
                  value="You are required to observe strict workplace compliance, data protection standards, and maintain complete confidentiality regarding enterprise technology." 
                  onChange={() => {}}
                />
              </p>
              <p>
                <EditableText 
                  multiline 
                  value={customClosing || "We look forward to welcoming you to the enterprise team and scaling digital products together."} 
      ...
      {/* Full-Bleed Tech Bottom Footer */}
      <div 
        className="w-full text-white px-6 py-3 mt-auto flex flex-col sm:flex-row items-center justify-between text-xs gap-2 shrink-0 font-sans print:bg-sky-900 print:text-white z-20"
